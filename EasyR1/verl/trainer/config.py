...
class TrainerConfig:
    ...
    def post_init(self):
        if self.save_checkpoint_path is None:
            ...
            self.save_checkpoint_path = os.path.join("ckpts", self.experiment_name)
...
class PPOConfig:
    ...
    # # -- Misc
    predoutput_ratio: float = 1
    predoutput_length_weight: float = 0.2
    charset: str = "basic"
    n_char_augment: int = 1
    dataset_ratio: float = 1.0
    stderr_weight: float = 0.5
    prob: float = 0.1
    max_num_corrupted_lines: int = 2
    opc_ddownsample: int = 1

    use_logical_augment: bool = True
    use_digit_augment: bool = True
    reward_stderr: bool = True # for loss only
    supported_error_types: str = "index error, value error, name error, type error, key error, zero division error"
    
    # supported_error_types = [e.strip() for e in supported_error_types.split(",") if e.strip()]

    # print("⚠️")
    # print(f"use_logical_augment: {use_logical_augment}")
    # print(f"use_digit_augment: {use_digit_augment}")
    # print(f"pred_stderr: {pred_stderr}")
    # print(f"supported_error_types: {supported_error_types}")
